...
只需用 `#decks` 標記一個檔案。您寫的每一個標題都會變成卡片的正面；其下方的段落都會變成背面。表格、圖片遮罩 (Image occlusion) 和 `==填空==` (Cloze) 也能以同樣的方式運作。卡片複習排程由 FSRS —— 現代的間隔重複演算法來處理。
...
- **您的筆記就是牌組。** 標記一個檔案，每個標題都會變成正面，每個段落變成背面。如果您習慣使用 Anki，那麼在這裡您不需要重複編寫兩次。
...
3. 寫一個標題，然後在其下方寫一段內文。需要多少張卡片就重複多少次：
...
<details>
<summary><b>標題 + 段落</b> — 預設格式。每個標題都是正面，內文下方是背面。</summary>
