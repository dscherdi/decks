...
| --- | --- |
| **Basic** model (≤ 2 fields) | Default `header-paragraph`; **escalates to a table** when the answer is a single paragraph (and any notes are too), or when the note has no real front (the front media becomes the front cell instead of a `## Card <id>` header). Table-routed basics aggregate into one table per deck **per column structure** (a 2-col and/or a 3-col table) — `AnkiCollectionParser.buildBasicCard` (`tableLayout`) + `AnkiDeckRenderer.renderTableSections` |
