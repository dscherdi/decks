...
## ✅ Latest: Exam decks — schema v39
- **Exam mode**: a drawn set of questions answered in one sitting with a graded results screen (per-question breakdown, optional pass score and countdown). Question kinds: multiple-choice (new authoring format), type-in (header-paragraph backs and table rows), and cloze-as-type-in (full line shown, `==highlights==` become inline inputs; hidden-context mode shows only the target line). Headless engine in core: `ExamAttempt` + `buildExamPool`/`drawExamQuestions` (`packages/decks-core/src/services/ExamAttempt.ts`), grading in `ExamGrading.ts` (`exact` / `tolerant` — <4 chars exact, ≥4 distance ≤1 or >85% similarity — / `self`; 120-char gradability ceiling surfaces as a manager badge via `CardHealth` exam context, ungradable cards are excluded from pools).
- **MCQ parsing is profile-gated**: `examEnabled` is a new `parsingAffected` profile flag (5th positional parser arg). In exam-enabled decks a heading + task list parses as a `multiple-choice` card (`ExamClassifier.classifyExamBody`: mcq/invalid/plain; tolerates trailing thematic breaks; rejects mixed/nested/single-option/no-correct bodies with per-reason badges). Card `back` stays byte-identical to the header-paragraph fallback, so toggling the flag flips card type without content churn. New anchor role `q` (own-line token after a blank line — `AnchorStamper`/`FlashcardWriter` carry it; `rekeyBindings` generalized via `rekeyCardKey`); dormant `h`/`q` tokens are mutually inert, so review history parks and self-heals across toggles (`countReviewedCardsBecomingQuestions` powers the type-flip confirm).
- **Schema v39**: `deck_profiles.exam_enabled`/`exam_settings` (JSON: count, time limit, pass score, shuffle q/o, feedback end|immediate, selection random|sequential, typed grading, option labels); new tables `exam_sessions` + `exam_answers` — **no FKs on `exam_answers` and commit-marker persistence**: `completeExamSession` writes answers first, session row last, all `INSERT OR IGNORE` with deterministic ids (`${sessionId}:${ordinal}`), so a torn write is invisible and merge-safe without cross-worker transactions. Sync op `exam_session_complete` (`SyncLog.handlers.ts`) replays the same way; both binary merge paths (worker `mergeAppendOnly` + `MainDatabaseService` ATTACH/fallback) union the tables; backup restore copies them.
- **Exams preset**: seeded profile `profile_exams` ("Exams", headerLevel 2, cloze ON, exam ON, new-cards limit 0) + one-time `<deckTag>/exams` tag mapping behind `settings.examsPresetMappingDone`. Seeds are `INSERT OR IGNORE` — existing rows are never updated.
- **UI**: `FlashcardExamModal.svelte` (free navigation, navigator chips, timer, letter/number labels, results) hosted by `ExamModalWrapper`/`ExamView` (modal vs tab follows `reviewDisplayMode`); `ExamSetupModal` prefilled from profile exam defaults; deck row click + ⋮ "Start exam" route on `profile.examEnabled`; MCQ cards are also interactive in normal review (`FlashcardReviewModal`). Demo deck `ExamDeckTemplate.ts` (flat H2-question structure) auto-created once (`hasCreatedExamDeck`) + `create-exam-demo-deck` command; all 13 locales extended.
- Key files: `packages/decks-core/src/services/ExamAttempt.ts`, `ExamClassifier.ts`, `ExamGrading.ts`, `FlashcardParser.ts`, `utils/anchors.ts`, `database/schemas.ts` (v39), `SyncLog.handlers.ts`; `apps/obsidian-plugin/src/components/exam/*`, `services/AnchorStamper.ts`, `database/BaseDatabaseService.ts`. Tests: `ExamAttempt.test.ts`, `ExamClassifier.test.ts`, `ExamDeckTemplate.test.ts`, `exam-schema.test.ts` (v38→v39 migration), `exam-results.test.ts` (commit-marker invisibility, two-device merge, op replay parity, backup). Design doc: `docs/EXAM_DECKS.md`.

## ✅ Durable suspend/bury state (card_state_overlays) — schema v38
