...
- **Saving the database can no longer damage it.** The database was previously rewritten in place, so a save interrupted partway — by a quit, by the machine sleeping, or by a cloud client swapping the file underneath — could leave a truncated file and no intact copy. The result was "database disk image is malformed" on next launch, with the whole review history lost. Saves now write to a temporary file and replace the database only once it is complete, so an interrupted save leaves the previous database untouched. Backups are written the same way, and a save interrupted at the moment of replacement is completed on the next launch.

  If you keep your vault in iCloud, Dropbox or similar, it is also worth marking the vault **Keep Downloaded** so the sync client cannot evict files while they are being written.
