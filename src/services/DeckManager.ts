...
export class DeckManager {
  ...
  constructor(
    ...
    this.flashcardSynchronizer = new FlashcardSynchronizer(db as unknown as DatabaseInterface);
      ...
      for (const [tag, files] of decksMap) {
        for (const file of files) {
          ...
          if (existingDeck) {
            ...
            this.debugLog(
              `Creating new deck: "${deckName}" with ID: ${this.generateDeckId(
                filePath,
  ...
  async syncFlashcardsForDeck(
    ...
    const dbHasWorkerMethod =
      typeof (this.db as IDatabaseService & { syncFlashcardsForDeckWorker?: unknown }).syncFlashcardsForDeckWorker === "function";

    if (dbHasWorkerMethod) {
      // Use worker for parsing and database operations
      try {
        const result = await (this.db as IDatabaseService & {
        ...
        this.performanceLog(
          `Worker sync completed for deck: ${
            deck.name
          } in ${formatTime(totalDeckSyncTime)} (${
            result.parsedCount
          } flashcards, ${result.operationsCount} operations)`,
        );
  ...
  private async syncFlashcardsForDeckMainThread(
    ...
    try {
      ...
      this.debugLog(
        `Deck sync completed in ${formatTime(deckSyncTime)} for deck: ${
          deck.name
        }`,
      );
    } catch (error) {
