            ...
            onProgress: (progress) => {
                ...
                if (!this.progressTracker.isVisible()) {
                    const message = forceSync
                        ...
                        : "🔄 Syncing flashcards...";
                    this.logger.debug(
                        `Showing initial progress notice: ${message}`
                    );
        ...
        try {
            ...
            for (let i = 0; i < decks.length; i++) {
                ...
                this.logger.debug(
                    `${
                        forceSync ? "Force s" : "S"
                    }yncing flashcards for deck: ${deck.name} (${
                        deck.filepath
                    })`
                );
            ...
            const flashcardSyncTime =
                performance.now() - flashcardSyncStartTime;
            ...
            if (decks.length > 0) {
                ...
                const avgFlashcardTime =
                    totalFlashcards > 0
                        ? flashcardSyncTime / totalFlashcards
                        : 0;
                this.logger.performance(
