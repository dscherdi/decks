...
describe("FSRS Algorithm Integration Tests", () => {
  ...
  beforeEach(async () => {
    ...
    const mockBackupService = undefined;
    const mockStatisticsService = {} as any;
  ...
  describe("New Card Learning Progression", () => {
    ...
    it("should handle Again rating on new card", async () => {
      const now = new Date();
      const flashcard = await scheduler.rate(newFlashcard.id, "again", now);
    ...
    it("should handle Hard rating on new card", async () => {
      const now = new Date();
      const flashcard = await scheduler.rate(newFlashcard.id, "hard", now);
    ...
    it("should handle Good rating on new card", async () => {
      const now = new Date();
      const flashcard = await scheduler.rate(newFlashcard.id, "good", now);
    ...
    it("should handle Easy rating on new card", async () => {
      const now = new Date();
      const flashcard = await scheduler.rate(newFlashcard.id, "easy", now);
    ...
    it("should maintain monotonic intervals across ratings", async () => {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        await scheduler.rate(card.id, ratings[i], now);
  ...
  describe("Review Card Progression", () => {
    ...
    it("should handle lapse (Again) on review card", async () => {
      const now = new Date();
      const updatedCard = await scheduler.rate(
        reviewFlashcard.id,
        "again",
        now,
      );
    ...
    it("should handle successful review progression", async () => {
      const now = new Date();
      const flashcard = await scheduler.rate(reviewFlashcard.id, "good", now);
    ...
    it("should calculate retrievability correctly", async () => {
      ...
      const testCases = [
        { elapsedDays: 1, expectedRetrievability: { min: 0.7, max: 1.0 } },
        { elapsedDays: 5, expectedRetrievability: { min: 0.3, max: 0.8 } },
        { elapsedDays: 10, expectedRetrievability: { min: 0.1, max: 0.5 } },
      ];
      ...
      for (const testCase of testCases) {
        ...
        await db.createFlashcard(card);
        await scheduler.rate(card.id, "good", now);
  ...
  describe("FSRS Parameter Compliance", () => {
    it("should respect request retention in interval calculation", async () => {
      ...
      const highScheduler = new Scheduler(
        db,
        {} as any,
        {} as any,
        "",
      ...
      const lowScheduler = new Scheduler(
        db,
        {} as any,
        {} as any,
        "",
    ...
    it("should enforce maximum interval limits", async () => {
      ...
      const now = new Date();
      await scheduler.rate(card.id, "easy", now); // Easy rating

      const updatedCard = await db.getFlashcardById(card.id);
      expect(updatedCard!.interval).toBeLessThanOrEqual(30 * 24 * 60); // 30 days in minutes
    });
  });
  ...
  describe("Scheduler Integration", () => {
    ...
    it("should generate scheduling previews correctly", async () => {
      ...
      const now = new Date();
      await scheduler.rate(card.id, "easy", now);
      const preview = await scheduler.preview(card.id, now);
    ...
    it("should handle review session workflow", async () => {
      ...
      while (reviewedCount < 3 && reviewedCount < cards.length) {
        await scheduler.rate(cards[reviewedCount].id, "good", now);
        reviewedCount++;
      }
  ...
  describe("Data Integrity and Persistence", () => {
    it("should maintain FSRS data consistency across operations", async () => {
      ...
      expect(finalCard!.repetitions).toBe(0);
      ...
      for (let i = 1; i < reviewLogs.length; i++) {
        expect(reviewLogs[i].newRepetitions).toBeGreaterThan(
          reviewLogs[i - 1].newRepetitions,
        );
      }
    });
