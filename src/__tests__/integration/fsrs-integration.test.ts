...
describe("FSRS Algorithm Integration Tests", () => {
    ...
    describe("New Card Learning Progression", () => {
        ...
        it("should maintain monotonic intervals across ratings", async () => {
            ...
            for (let i = 0; i < 4; i++) {
                const card = DatabaseTestUtils.createTestFlashcard(
                    testDeck.id,
                    {
            ...
            if (cards.length >= 4) {
                expect(cards[0].interval).toBeLessThanOrEqual(
                    cards[1].interval
                );
                expect(cards[1].interval).toBeLessThanOrEqual(
                    cards[2].interval
                );
                expect(cards[2].interval).toBeLessThanOrEqual(
                    cards[3].interval
                );
            }
        });
    ...
    describe("Review Card Progression", () => {
        ...
        beforeEach(async () => {
            ...
            reviewFlashcard = DatabaseTestUtils.createTestFlashcard(
                testDeck.id,
                {
                    state: "review",
                    repetitions: 2,
            ...
            for (const testCase of testCases) {
                const card = DatabaseTestUtils.createTestFlashcard(
                    testDeck.id,
                    {
                        ...
                        lastReviewed: new Date(
                            now.getTime() -
                                testCase.elapsedDays * 24 * 60 * 60 * 1000
                        ).toISOString(),
                    }
                );
                await db.createFlashcard(card);
    ...
    describe("FSRS Parameter Compliance", () => {
        it("should respect request retention in interval calculation", async () => {
            ...
            const card2 = DatabaseTestUtils.createTestFlashcard(
                lowRetentionDeck.id,
                { state: "review", stability: 5.0, repetitions: 2 }
            );
    ...
    describe("Scheduler Integration", () => {
        ...
        it("should handle review session workflow", async () => {
            ...
            const session = await scheduler.startReviewSession(
                testDeck.id,
                now
            );
