  ...
  describe("Review Session Simulation", () => {
    ...
    it("should simulate review with multiple ratings", async () => {
      ...
      for (let i = 0; i < reviewCount; i++) {
        ...
        if (card) {
          ...
          const timeElapsedMs = Math.floor(Math.random() * 10000) + 2000; // 2-12 seconds
    ...
    it("should update card counts after reviews", async () => {
      ...
      const initialNewCount = (await db.getNewCardsForReview(testDeck.id))
        .length;
      const initialDueCount = (await db.getDueFlashcards(testDeck.id)).length;
      ...
      const newNewCount = (await db.getNewCardsForReview(testDeck.id)).length;
    ...
    beforeEach(async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (card) {
          ...
          await scheduler.rate(card.id, ratings[i % 3], 3000 + i * 500);
        }
      }
      await scheduler.endReviewSession(session.sessionId);
    });
    ...
    it("should categorize reviews by difficulty (rating distribution)", async () => {
      ...
      allLogs.forEach((log) => {
        if (log.ratingLabel in ratingCounts) {
          ratingCounts[log.ratingLabel as keyof typeof ratingCounts]++;
        }
      });
