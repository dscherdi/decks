...
export default class DecksPlugin extends Plugin {
    ...
    async onload() {
        ...
        try {
            ...
            if (this.settings?.ui?.enableNotices !== false) {
                new Notice(
                    "Failed to load Decks plugin. Check console for details."
                );
            }
        }
    }
    ...
    async handleFileChange(file: TFile) {
        ...
        if (hasFlashcardsTag) {
            ...
            if (existingDeck) {
                ...
                const newTag =
                    allTags.find((tag) => tag.startsWith("#flashcards")) ||
                    "#flashcards";
                ...
                const newTag =
                    allTags.find((tag) => tag.startsWith("#flashcards")) ||
                    "#flashcards";
                await this.deckSynchronizer.createDeckForFile(
                    file.path,
                    newTag
                );
                await yieldToUI();
    ...
    async performInitialSync() {
        try {
            ...
            this.logger.performance(
                `Initial sync completed successfully in ${formatTime(
                    totalTime
                )}`
            );
        } catch (error) {
    ...
    async handleFileRename(
        ...
        if (file instanceof TFile && file.extension === "md") {
            ...
            if (oldDeck) {
                ...
                this.logger.debug(
                    `File renamed from ${oldPath} to ${file.path}`
                );
                this.logger.debug(
                    `Updating deck ID from ${oldDeckId} to ${newDeckId}`
                );
                ...
                await this.deckManager.updateFlashcardDeckIds(
                    oldDeckId,
                    newDeckId
                );
    ...
    private async updateLastKnownDatabaseMtime(
        ...
        try {
            ...
            this.logger.debug(
                "Failed to update lastKnownDatabaseMtime:",
                error
            );
        }
    }
