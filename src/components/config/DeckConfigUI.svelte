<script lang="ts">
  ...
  export let onconfigChange: ((config: DeckConfig) => void) | undefined =
    undefined;
  ...
  onMount(() => {
    ...
    if (reviewOrderContainer) {
      new Setting(reviewOrderContainer)
        ...
        .setDesc("Order in which review cards are presented during study")
    ...
    if (headerLevelContainer) {
      new Setting(headerLevelContainer)
        ...
        .setDesc("Which header level to use for header-paragraph flashcards")
        .addDropdown((dropdown) =>
          dropdown
            .addOption("1", "H1 (#)")
            .addOption("2", "H2 (##)")
...
<div class="decks-deck-config-ui">
  ...
  <div class="decks-deck-settings-section">
    ...
    <div class="decks-setting-item-description-global">
      These settings control the spaced repetition algorithm for this deck.
    </div>
