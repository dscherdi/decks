<script lang="ts">
  ...
  function handleRowClick(card: CardData) {
...
<div class="flashcards-table-container notion-like-table">
  <table class="flashcards-table">
    <thead>
      <tr>
        {#each [
          ...
          { id: "nextReview", text: "Next Review" },
          { id: "actions", text: "Actions" }
        ] as header}
          <th>
            {#if header.id !== 'actions'}
              <div 
                class="flashcards-sortable-header"
                on:click={() => handleHeaderClick(header.id)}
              >
                {header.text}
                <span class="flashcards-sort-icon">
                  {#if sortField === header.id}
                    <svg viewBox="0 0 100 100" class="sort-icon" width="10" height="10">
    ...
    <tbody>
      {#if filteredCards.length === 0}
        <tr class="notion-like-empty-row">
          {#each Array(3) as _, i}
            <td class="notion-like-empty-td"></td>
          {/each}
        </tr>
        <tr>
          <td colspan="5" class="notion-like-empty-cell">
      ...
      {:else}
        {#each filteredCards as card}
          <tr on:click={() => handleRowClick(card)}>
            <!-- Title column -->
            <td>
              <div class="flashcards-card-title notion-like-cell">
                {card.title}
              </div>
            </td>
            ...
            <!-- Tags column -->
            <td>
              <div class="flashcards-tags notion-like-tags">
                {#if card.tags && card.tags.length > 0}
                  ...
                  {/each}
                {:else}
                  <span class="flashcards-no-tags">No tags</span>
                {/if}
              </div>
            </td>
            ...
            <!-- Created date column -->
            <td>
              <div class="flashcards-date notion-like-cell">
                {card.created ? new Date(card.created).toLocaleDateString() : 'Unknown'}
              </div>
            </td>
            ...
            <td>
              ...
              <div class={`flashcards-review-date ${status.class} notion-like-cell`}>
                {status.text}
              </div>
            </td>
            ...
            <!-- Actions column -->
            <td>
              <div class="flashcards-actions notion-like-actions">
                <button 
